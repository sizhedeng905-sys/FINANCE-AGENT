...
mc alias set staging http://minio:9000 "$root_user" "$root_password"
...
mc admin user add staging "$runtime_user" "$runtime_password" >/dev/null 2>&1 \
  || mc admin user enable staging "$runtime_user"
...
mc admin policy attach staging finance-agent-runtime --user "$runtime_user"
