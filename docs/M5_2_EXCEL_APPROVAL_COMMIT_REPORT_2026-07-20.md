...
> 历史状态说明（2026-07-21）：本文记录 M5.2 当时对“错误行不得部分发布”和双人批准的实现与测试，不再单独作为 Excel 暂存隔离/完整性 P0 的关闭证据。后续攻击测试证明旧实现的 `pending_confirm` 记录仍可被通用 records API 读取和修改，且最终发布没有逐记录内容/来源镜像及实际更新行数围栏。当前结论与修复证据以 [`P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md`](P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md) 为准；本文的历史测试数字不改写。
...
M5.2 在当时关闭了 Excel `valid_rows_only` 部分发布问题；该结论不包含后来重新发现的暂存隔离与发布完整性 P0。当前业务规则是：
...
- 当时设计要求 staging 对报表不可见；后续审计证明仅依赖 `pending_confirm` 状态不足以隔离所有通用 records 路径。当前实现已改为显式 publication status，并增加逐记录哈希、数据库失效触发器和精确更新行数围栏。只有全部条件一致时，最终单一事务才统一发布：
