...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1 已关闭唯一工程 P0，R2-R5 已关闭日志泄露、固定对象容量伪装、恢复完整性和镜像身份 P1，R6.1-R6.3 已关闭 Excel 预览全量响应、项目模板并发锁和重复候选窗口空转风险，剩余 2 个 P1/条件 P1 继续按风险顺序处理；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6 执行中 | R6.1 预览、R6.2 项目模板锁和 R6.3 重复窗口已关闭；下一步处理 Decimal 精度、幂等/H 矩阵，再处理多实例闸门与治理证据 |
...
继续 R6.4 Decimal/金额阈值精度：追踪 DTO、JSON、Prisma Decimal、规则计算和序列化，先复现大额小数经 JavaScript `number` 失真的红灯，再实现规范十进制字符串与安全旧 numeric 子集。随后完成 R6.5 幂等清单和 R6.6 H01/H02/H07 行为矩阵；完成 R6 后继续 R7/R8，再按主任务书优先级进入 M0-M8。
