...
当前批次：真实业务数据 B0-B7 工程交付完成，等待财务 UAT 签字与 OCR 标签
...
当前自动化证据：18/18 Prisma migrations；17/17 Jest suites、183/183 tests；30/30 真实 PostgreSQL；14/14 Playwright；前后端 build。真实 Qwen/Paddle 运行稳定性已验收；OCR 字段准确率和 L3 会计真值仍需财务签字。
...
- B7：前后端 production build、17/17 Jest suites、183/183 tests、30/30 PostgreSQL integration、14/14 Playwright、Prisma、hygiene、依赖审计和 112 份原件复扫全部通过。可签字交付物见 `docs/B7_FINANCE_UAT_ACCEPTANCE.md`；工程状态为完成，业务状态保持 `awaiting_finance_signoff`，OCR 状态保持 `awaiting_labels`。
...
当前自动化证据：18/18 Prisma migrations；17/17 Jest suites、183/183 tests；30/30 真实 PostgreSQL；14/14 Playwright；前后端 build；根目录与后端生产依赖审计均为 0 vulnerabilities。四套本地模型资产完整，真实 GPU 稳定性通过，OCR 准确率仍待标签。
...
| AI/模型运行时 | 显式 Mock/API 聊天；会话和消息分页；模型运行时通过受保护 API 查看部署、路由和健康 | 结构化工具、指定月份/同比环比、数字 grounding、受控 fallback、OpenAI-compatible、常驻/按需编排 | 模型部署、任务路由、会话消息和调用日志均持久化 | 72 条 Qwen 基准、183 个单测、30 个 PostgreSQL 集成测试通过 | Qwen 文本与 OCR 常驻，VL/Embedding 按需 | B5 有效回答通过；原始模型高 fallback，必须保留结构化降级 |
| E2E 验收 | Playwright 驱动 API/Mock 两套前端，覆盖四角色、完整审批、XLS/XLSX、OCR、数据中心、报表、错误与安全运行 | 专用测试启动脚本，统一错误、CORS、安全头和 readiness 可断言 | 独立 `_test` PostgreSQL 自动 migrate/seed，精确清理 E2E 工单、导入/OCR任务、记录与文件 | 183 个单元测试、30 个 PostgreSQL 集成测试、14 个 Playwright E2E | API 与 Mock 均有自动化证据 | 审计修复和真实数据 B0-B7 工程回归通过，GitHub CI job 已配置 |
...
| 后端 `npm run build` | 通过 |
| 后端 `npm test -- --runInBand` | 17/17 suites，183/183 tests 通过 |
