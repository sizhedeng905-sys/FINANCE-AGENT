...
进入 R6 后端资源与并发边界：先以失败测试复现 Excel 预览全量返回、项目模板变更 TOCTOU、重复时间窗未生效和 Decimal 阈值失真，再做最小修复与 PostgreSQL/浏览器回归。完成 R6 后继续 R7/R8，之后按主任务书优先级进入 M0-M8 复用审计和统一 AI 映射流水线。
