...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1-R6.6 已关闭工程 P0/P1 主链，R7.1 已完成 AI 审计内容/元数据分离和只允许 dry-run 的数据生命周期盘点；R7.2 step-up/职责分离与 R8 尚待推进；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6、R7.1 完成 / R7.2 下一步 | R7.1 的 9 类 retention dry-run、legal hold、lease/恢复、匿名证据和 AI 日志元数据分离通过；H12/H14 禁止真实删除；下一步收紧 step-up/职责分离 |
...
进入 R7.2 step-up/职责分离：先审计现有 token 的 action/resource/session 绑定、消费并发、角色/账号/密码变化失效和高风险动作覆盖，再实现统一 guard 与攻击性 PostgreSQL 测试。MFA、自审批矩阵、双人复核和应急账号仍由 H10 决定；完成后进入 R8 分层 CI，再继续 M0-M8。
