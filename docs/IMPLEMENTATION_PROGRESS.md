...
- 生产全局请求限流与登录口令限流已由 Redis 共享。R9.1 使用原子 Lua、摘要身份键、Redis 时钟租约、幂等完成和断连失败关闭；双实例攻击 4/4、后端 419/419 unit、PostgreSQL/Redis 101/101 integration 与 build 本地通过。上传准入和模型并发闸门仍为进程内状态，所以 B8-09 Compose 继续固定单 API、单 Worker；详见 `docs/R9_1_SHARED_LOGIN_RATE_LIMIT_REPORT_2026-07-21.md`。
