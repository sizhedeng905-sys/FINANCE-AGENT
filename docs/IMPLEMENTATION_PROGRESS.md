...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行。M8 提交 `30c6ead` 已推送；CodeQL run `29752262976` 通过，Build run `29752263099` 的业务 build、410 个单测、97 个 PostgreSQL 集成和 17 个 Playwright 用例通过，随后由供应链门禁检出旧 Nginx 的可修复 Critical。M8.1 已固定升级到本地实扫通过的 `nginx:1.30.4-alpine3.24`，等待远端新 run。负责人问卷 Q01-Q30 已映射到 H01-H16，H01 明确按每行明细，但签名、执行清单和真实证据未关闭；retention 仍只允许 dry-run，step-up 默认关闭。AI 映射补充任务 M0-M8 已完成非生产工程与合成验收；受保护 Prompt Catalog、真实口径/准确率、目标 Staging 和人工签字继续按真实状态阻塞。
...
- M8.1 先实测 `nginx:1.28.3-alpine`，当前 Grype 数据库仍发现 OpenSSL Critical 修复版本缺口；最终固定到官方稳定 `nginx:1.30.4-alpine3.24@sha256:97d490...e5b46`。真实前端镜像和 R5 夹具各生成 72 包 SPDX，`--only-fixed --fail-on critical` 均为 0；17 个供应链攻击用例、Nginx 19/50/50+1 MiB 边界、runtime 4/4、部署/CI 契约 19/19、前后端 build 通过。详见 `docs/M8_1_NGINX_CI_SECURITY_REFRESH_2026-07-20.md`。
...
M0-M8 非生产工程与合成验收已经收口。下一检查点是推送 M8.1 并观察远端 Build/CodeQL；随后只处理真实未决工作：补齐 Prompt Catalog 正文并逐字审计 registry，按 H01-H16 收集汇总行样例、财务/OCR/AI 真值和正式签字，在 H13/H14 指定的目标 Linux 执行 release、restore、RPO/RTO、告警和跨版本 rollback。PR #4 在 H15/H16 前保持 Draft，不 merge、不标记 Ready。
