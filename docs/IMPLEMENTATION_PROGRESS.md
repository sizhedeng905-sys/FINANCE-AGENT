...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1-R8.6 本机工程门禁已完成，retention 只允许 dry-run，step-up 默认关闭且正式职责分离仍待 H10；下一步验证 R8.7 OpenSSL 最终镜像并收口 R8；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6-R8.6 本机门禁完成 / R8.7 执行中 | 完整 release、7 项 sealed gate、日志和同 manifest rollback 已通过；下一块验证带 OpenSSL 3 的最终后端镜像 |
...
- rollback 输出真实暴露后端运行镜像缺少 OpenSSL CLI，Prisma 回退猜测 `openssl-1.1.x`。R8.7 给 Node build/runtime 共享基础层安装 OpenSSL/CA，CI 在最终 UID 10001 镜像中执行 OpenSSL 与 Prisma 探针并拒绝 detect warning。本地镜像构建第二次通过（第一次 Debian mirror 502），OpenSSL 3.0.20、Prisma 6.19.3 和 `debian-openssl-3.0.x` target 均实测；当前 commit 的完整 release 仍待重跑。
...
提交 R8.7 后重跑最终本地 Staging release，确认 migration/rollback 不再出现 Prisma OpenSSL 检测告警，复核运行日志并清理本项目资源。随后收口 R8，继续 M0-M8 复用审计；目标 Linux、跨版本/正式恢复/RPO/RTO 和真实 GPU 准确率仍分别受 H13/H14 与 H04-H09/H12/H16 阻断。
