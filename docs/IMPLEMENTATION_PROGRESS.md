...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1 已关闭唯一工程 P0，R2-R5 已关闭日志泄露、固定对象容量伪装、恢复完整性和镜像身份 P1，R6.1-R6.5 已关闭 Excel 预览全量响应、项目模板并发锁、重复候选窗口空转、规则金额阈值精度和财务写入口幂等工程风险，剩余 1 个条件 P1 继续按风险顺序处理；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6 执行中 | R6.1 预览、R6.2 项目模板锁、R6.3 重复窗口、R6.4 Decimal 阈值和 R6.5 财务写幂等已关闭；下一步完成 H 行为矩阵，再处理多实例闸门与治理证据 |
...
继续 R6.6 H01-H16 行为矩阵：逐项把人工决策、当前保守默认、配置点、自动测试和禁止声明对应起来；不把缺失签字解释为默认批准。完成 R6 后继续 R7/R8，再按主任务书优先级进入 M0-M8。
