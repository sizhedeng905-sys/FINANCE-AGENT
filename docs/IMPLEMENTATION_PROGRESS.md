...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1 已关闭唯一工程 P0，R2-R5 已关闭日志泄露、固定对象容量伪装、恢复完整性和镜像身份 P1，R6.1-R6.2 已关闭 Excel 预览全量响应和项目模板并发锁风险，剩余 3 个 P1/条件 P1 继续按风险顺序处理；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6 执行中 | R6.1 预览与 R6.2 项目模板锁已关闭；下一步处理重复窗口、Decimal 精度、幂等/H 矩阵，再处理多实例闸门与治理证据 |
...
继续 R6.3 重复候选时间窗：先证明 `duplicate_submission.windowDays` 当前是否真正约束候选日期范围，覆盖窗口内、边界日、窗口外、跨来源和未来日期，再做最小修复。随后依次完成 R6.4 Decimal 阈值、R6.5 幂等清单和 R6.6 H01/H02/H07 行为矩阵；完成 R6 后继续 R7/R8，再按主任务书优先级进入 M0-M8。
