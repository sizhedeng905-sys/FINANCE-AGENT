...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1-R7.2、R8.1 与 R8.2 条件验收自动化已完成，retention 只允许 dry-run，step-up 默认关闭且正式职责分离仍待 H10；下一步执行当前 commit 的完整本地 release/恢复/回滚；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6、R7.1、R7.2、R8.1、R8.2 条件门禁完成 / 实际执行中 | 每次 CI 构建扫描应用镜像；scheduled/manual Staging 与手工 GPU L0 路径已落地；下一块执行当前 commit 的完整本地 release/恢复/回滚 |
...
- R8.2 红灯确认完整 Compose release/restore/rollback、Python 适配器依赖契约和 GPU 模型运行路径没有 GitHub 条件门禁；新增 scheduled/manual Staging workflow 与仅手工 GPU L0 workflow，并加入运行日志泄露检测、资源预检/清理和受限证据上传。actionlint 零告警，Python 3.10.19 全依赖 `pip check` 与 8/8 契约通过，日志/配置算法各 3/3，18 镜像 Staging scope lock 通过。首次 `staging:check` 又暴露旧 `.env` 配置漂移；现只同步 19 个仓库管理项、保留运维项和 secret，第二次初始化更新 0 项。后端 38/38 suites、349/349 tests、前后端 build、638 文件卫生及两套 0 vulnerability 审计通过；完整 release 与 GPU L0 尚未执行，详见 `docs/R8_2_CONDITIONAL_ACCEPTANCE_AUTOMATION_REPORT_2026-07-18.md`。
- R8.2 首次完整 release 在构建前失败：Compose `--ignore-buildable` 仍尝试拉取被 `minio-init` 共享的本地 backup 镜像。现场没有启动容器或写入数据；release 现只允许拉取 `redis/clamav/gateway/grafana/loki` 五个固定第三方服务，修复后完整重跑待干净提交执行。
...
执行当前 commit 的完整本地 Staging release：构建并锁定 18 个镜像，完成供应链扫描、Compose 启动、API/浏览器 smoke、隔离恢复、运行日志检测和同 manifest rollback，最后确认容器/网络/卷残留为 0。通过后收口 R8，继续 M0-M8 复用审计；目标 Linux、正式恢复/RPO/RTO 和真实 GPU 准确率仍分别受 H13/H14 与 H04-H09/H12/H16 阻断。
