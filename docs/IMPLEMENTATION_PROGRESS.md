...
- B2 尚未完成：超过 50 MiB 文件的产品通道；当前文件服务仍会持有受 50 MiB 上传上限约束的压缩 Buffer。

当前自动化证据：16/16 Prisma migrations；15/15 Jest suites、94/94 tests；28/28 真实 PostgreSQL；14/14 Playwright；前后端 build。以上只证明结构、安全和业务回归，真实 PaddleOCR/Qwen 推理准确率仍未验收。
...
| Excel | 显式 Mock/API Repository、真实上传、Sheet/表头/公式缓存选择、媒体隔离提示、后台进度/取消、任务列表、列映射、字段建议、错误预览和确认页 | 工作簿检查、多 Sheet/隐藏 Sheet门禁、合并表头、共享公式、媒体隔离；5000 行同步边界，5001-50000 行后台分批；旧 `.xls` 受限子进程转换 | ImportTask/Sheet/Column/Row、Profile/Decision/Suggestion；第 16 个 migration 新增后台配置、执行模式、进度和恢复字段 | 94 个普通测试、28 个 PostgreSQL 集成测试、14 个浏览器回归；15 份 XLS 与 5001/30196 行门禁通过 | API 模式读取真实文件；Mock 模式显式可选 | XLS/XLSX 主闭环完成；超过 50 MiB 通道待定 |
...
| E2E 验收 | Playwright 驱动 API/Mock 两套前端，覆盖四角色、完整审批、XLS/XLSX、OCR、数据中心、报表、错误与安全运行 | 专用测试启动脚本，统一错误、CORS、安全头和 readiness 可断言 | 独立 `_test` PostgreSQL 自动 migrate/seed，精确清理 E2E 工单、导入/OCR任务、记录与文件 | 94 个普通测试、28 个 PostgreSQL 集成测试、14 个 Playwright E2E | API 与 Mock 均有自动化证据 | 审计修复和真实数据 B0/B1/B2 Excel 切片回归通过，GitHub CI job 已配置 |
...
| I 真实业务数据 | B0/B1 完成，B2 进行中 | 112 个文件只读结构基线、图片/PDF 安全、多 Sheet/表头/公式、媒体流式隔离、30196 行后台门禁与 15 份 `.xls` 已通过；超过 50 MiB 通道和真实模型准确率继续适配 |
