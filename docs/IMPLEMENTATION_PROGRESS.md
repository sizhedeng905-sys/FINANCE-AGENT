...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计继续收口。R9.1-R9.3B 已把登录、上传和模型执行控制共享化；其后 CR-002 至 CR-005 重新发现并关闭 Excel 未发布记录提前可见/可写、暂存内容篡改、来源竞态、最终少更新一行和慢批次误重试等 P0。CR-006 已关闭 P1-1 的 Prompt 真执行与 provenance 子块：版本化模板经受控 JSON 渲染后进入 Provider，请求/Schema/Provider/模型/结果哈希进入现有审计链。当前本地候选通过 435 个单测；P0 基线为强制 Redis 的 119 个 PostgreSQL 集成和 17 个 Playwright，本次 Prompt 专项为 7 个 PostgreSQL/Redis 用例；43 条 migration 的空库及 42→43 升级已验证。项目治理改为唯一负责人模式，H01-H16 只保留为历史问题索引；已回答决定不重复询问，未回答问题集中到 owner-input 台账并采用失败关闭默认。production-safe 空库 bootstrap 仍在推进；retention 仍只允许 dry-run，step-up 默认关闭；两个受保护 Prompt 文档仍为 0 字节并标记 `OWNER_CONFIRMATION_NEEDED`，真实口径/准确率为 `REAL_SAMPLE_NEEDED`，目标 Staging 为 `EXTERNAL_RESOURCE_NEEDED`。
...
| R11 最终交接 | 进行中 | Excel P0 与 P1-1 Prompt 执行/provenance 已通过并推送 Draft PR #4；下一子块是 production-safe bootstrap，保持 Draft、不 merge、不转 Ready |
...
- M5.2 当时按 H01 每行明细关闭了 Excel partial commit：普通错误明细不可排除，疑似汇总行必须财务处置，任何修改使旧校验失效。该阶段 403 unit、96 PostgreSQL、17 Playwright 及 36→37 migration 是历史证据，不覆盖后来发现的通用 API 暂存隔离与逐记录完整性问题；现行结论见 `docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md`。
- M6 完成 canonical ReportSnapshot：固定查询只读取 `confirmed + actual`，使用 repeatable-read、Decimal、分币种和来源版本/hash；AI 只能逐字选择服务端 Claim 白名单，值篡改、额外数字、虚构实体、原因/比较和 warning 遗漏均失败关闭。H06/H08 真实口径为 `REAL_SAMPLE_NEEDED`，报告复核角色为 `OWNER_CONFIRMATION_NEEDED`；详见 `docs/M6_REPORT_SNAPSHOT_GROUNDING_REPORT_2026-07-20.md`。
...
- 详细步骤与证据见 `docs/B8_09_STAGING_RUNBOOK.md`、`docs/B8_09_STAGING_REPORT.md` 和 `docs/RELEASE_CANDIDATE_AUDIT.md`。H12-H16 当前分别归入 `OWNER_CONFIRMATION_NEEDED`、`REAL_SAMPLE_NEEDED`、`EXTERNAL_RESOURCE_NEEDED` 或 owner UAT 门禁。
...
- 详细证据见 `docs/B8_06_SECURITY_HARDENING_REPORT.md`。H10/H11 的 step-up 与扫描 bypass 仍为 `OWNER_CONFIRMATION_NEEDED`。
...
- 详细证据见 `docs/B8_05_AI_CLAIM_GROUNDING_REPORT.md`。H08 标准答案为 `REAL_SAMPLE_NEEDED`，H12 Provider 清单为 `OWNER_CONFIRMATION_NEEDED / EXTERNAL_RESOURCE_NEEDED`。
...
- B7：前后端 production build、17/17 Jest suites、184/184 tests、30/30 PostgreSQL integration、14/14 Playwright、Prisma、hygiene、依赖审计和 112 份原件复扫全部通过。历史交付物见 `docs/B7_FINANCE_UAT_ACCEPTANCE.md`；工程状态为 `ENGINEERING_VERIFIED`，财务/OCR 真值为 `REAL_SAMPLE_NEEDED`。
