...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1-R7.2、R8.1 与 R8.2-R8.6 条件验收自动化已完成，retention 只允许 dry-run，step-up 默认关闭且正式职责分离仍待 H10；下一步重跑新鲜备份门禁修复后的完整本地 release/恢复/回滚；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6、R7.1、R7.2、R8.1、R8.2-R8.6 条件门禁完成 / 实际执行中 | 每次 CI 构建扫描应用镜像；scheduled/manual Staging 与手工 GPU L0 路径已落地；下一块重跑新鲜备份门禁修复后的完整本地 release/恢复/回滚 |
...
- 安全日志证据把泄露定位到 `minio-init/s3_access_key_id`，实际来源是 policy attach 成功输出。相关命令现只输出固定错误码；强制重建 `minio-init` 后运行日志比对 19 个 secret、586,356 bytes/2,887 行，结果 0 finding。完整 R8.6 发布重跑仍为 `not_run`；详见 `docs/R8_6_BACKUP_RELEASE_GATE_REPORT_2026-07-18.md`。
...
提交 R8.6 新鲜备份门禁后重跑完整本地 Staging release：重新构建并锁定 18 个镜像，完成供应链扫描、Compose 启动、API/浏览器 smoke、新鲜备份、隔离恢复、运行日志检测和同 manifest rollback，最后确认容器/网络/卷残留为 0。通过后收口 R8，继续 M0-M8 复用审计；目标 Linux、正式恢复/RPO/RTO 和真实 GPU 准确率仍分别受 H13/H14 与 H04-H09/H12/H16 阻断。
