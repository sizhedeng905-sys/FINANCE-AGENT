...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1 已关闭唯一工程 P0，R2-R5 已关闭日志泄露、固定对象容量伪装、恢复完整性和镜像身份 P1，R6.1-R6.6 已关闭 Excel 预览、项目模板锁、重复窗口、Decimal、财务写幂等及 H01/H02/H07 隐式政策风险；剩余 1 个条件 P1 继续按风险顺序处理；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6 完成 / R7 下一步 | R6.1-R6.6 全部工程门禁通过；H01/H02/H07 保持 Pending；下一步实现可 dry-run、无自动删除的数据生命周期框架和 step-up/职责分离基础设施 |
...
进入 R7.1 数据生命周期：盘点 AI message、Provider 请求/响应、工具调用、idempotency、OCR 中间结果、通知和审计增长面；先实现只读计划与 dry-run、批量上限、lease、legal hold 跳过和失败恢复，H12/H14 未批准时禁止自动删除。随后完成 R7.2 step-up/职责分离和 R8 分层 CI，再进入 M0-M8。
