...
- M8 完成最终证据收口：架构/API/E2E/本地运行/PR 指南和准备文档均更新到 M5-M7 真实边界；Prompt manifest/guard 定向 4/4 unit 与 3/3 PostgreSQL 通过；前后端 build、runtime 4/4、41 条 migration 空库及 40→41、222 indexes、89 foreign keys、708 文件卫生和两套 0 vulnerability 审计再次通过。受保护 Prompt Catalog 为空、GitHub 推送和 H 门禁仍未关闭，详见 `docs/M8_FINAL_EVIDENCE_AND_DRAFT_PR_HANDOFF_2026-07-20.md`。
...
M0-M8 非生产工程与合成验收已经收口，下一检查点只包含真实未决工作：网络恢复后正常推送并观察远端 Build/CodeQL；补齐 Prompt Catalog 正文后做逐字 registry 映射审计；按 H01-H16 收集汇总行样例、财务/OCR/AI 真值和正式签字；在 H13/H14 指定的目标 Linux 执行 release、restore、RPO/RTO、告警和跨版本 rollback。PR #4 在 H15/H16 前保持 Draft，不 merge、不标记 Ready。
