...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1 已关闭唯一工程 P0，R2-R5 已关闭日志泄露、固定对象容量伪装、恢复完整性和镜像身份 P1，R6.1-R6.4 已关闭 Excel 预览全量响应、项目模板并发锁、重复候选窗口空转和规则金额阈值精度风险，剩余 1 个条件 P1 继续按风险顺序处理；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6 执行中 | R6.1 预览、R6.2 项目模板锁、R6.3 重复窗口和 R6.4 Decimal 阈值已关闭；下一步处理幂等/H 矩阵，再处理多实例闸门与治理证据 |
...
继续 R6.5 幂等入口清单：逐项盘点财务写接口、现有 `IdempotencyKey`/事务/唯一约束、重放语义与冲突错误，先证明缺口再做最小公共基础设施。随后完成 R6.6 H01/H02/H07 行为矩阵；完成 R6 后继续 R7/R8，再按主任务书优先级进入 M0-M8。
