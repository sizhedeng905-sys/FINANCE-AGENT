...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计正在执行；R1-R7.2、R8.1 与 R8.2-R8.5 条件验收自动化已完成，retention 只允许 dry-run，step-up 默认关闭且正式职责分离仍待 H10；下一步重跑 PostgreSQL 远程 TLS 就绪修复后的完整本地 release/恢复/回滚；AI 映射补充任务 M0-M8 已纳入同一主线
...
| R6-R8 后端边界、治理、CI | R6、R7.1、R7.2、R8.1、R8.2-R8.5 条件门禁完成 / 实际执行中 | 每次 CI 构建扫描应用镜像；scheduled/manual Staging 与手工 GPU L0 路径已落地；下一块重跑数据库 TLS 修复后的完整本地 release/恢复/回滚 |
...
提交 PostgreSQL 远程 TLS 就绪修复后重跑完整本地 Staging release：重新构建并锁定 18 个镜像，完成供应链扫描、Compose 启动、API/浏览器 smoke、隔离恢复、运行日志检测和同 manifest rollback，最后确认容器/网络/卷残留为 0。通过后收口 R8，继续 M0-M8 复用审计；目标 Linux、正式恢复/RPO/RTO 和真实 GPU 准确率仍分别受 H13/H14 与 H04-H09/H12/H16 阻断。
