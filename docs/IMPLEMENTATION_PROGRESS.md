...
| M0-M8 AI 分类/映射/审批/快照补充 | M0-M7 工程通过 / M8 进行中 | Excel/OCR AI 只建议；双人审批、整批原子入账、不可变报告快照、严格 Claim grounding、攻击/并发/资源/降级门禁已有自动化证据；真实口径、准确率、Staging 和签字仍外部阻断 |
...
R8.7 完整 release 重验已在两次 Debian security 502 后标记 `blocked_external`，不再继续网络重试；恢复后需重新执行 migration/rollback 告警检查。M0-M1 已关闭，当前进入 M2 Prompt Registry、严格 Schema、AI mode 与 kill switch；目标 Linux、跨版本/正式恢复/RPO/RTO 和真实 GPU 准确率仍分别受 H13/H14 与 H04-H09/H12/H16 阻断。
