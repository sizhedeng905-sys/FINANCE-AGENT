...
| R9-SCALE-001 | P1 条件风险 | Multi-instance login/upload/model gates | 工程执行者 | R9.1-R9.3 已关闭登录、上传和模型执行门的本地工程子项；目标拓扑仍维持单 API/单 Worker | R9.1-R9.3 本地提交 | 三类控制均使用 Redis 原子预算、摘要键、服务器时钟租约和失败关闭；模型双实例 6/6、428/428 unit、当前强制 Redis 114/114 integration | engineering_verified | H13/H14；目标服务器多实例 release、恢复与回退未执行，不得宣称横向扩容已验收 |
...
| R9-CONFIRM-RECOVERY-001 | P1 条件风险 | Import final transaction timeout/retry | 工程执行者 | 默认 PostgreSQL 与远端 CI 均证明 30,196 行可能在全部暂存后卡入最终发布预算；旧逻辑会错误终态失败或超时 | R9.3A/R9.3B | P2028/P2034 有界租约恢复；租约心跳完整性预检与短原子发布；强制 Redis 114/114 integration；30,196/49,999 行 25.502/42.954 秒；暂存不可见且只发布一次 | engineering_verified_locally / remote_ci_required | H13 目标磁盘/WAL/checkpoint/p95/p99 容量验收；最新 head/checks 以 Draft PR #4 为准 |
...
| B8-STAGING-003 | P1 | B8-09/RC | 多实例登录、上传与模型闸门 | 全局请求、登录、上传与模型执行门均已共享；目标 Compose 尚未做多实例部署 | R9.1-R9.3 已完成本地迁移；Staging 继续单 API/单 Worker，等待 H13 目标拓扑 | 三类双实例并发/重启/租约/Redis 故障均通过；模型专项 6/6，当前完整 integration 114/114 | engineering_verified | H-13/H-14 仍阻断目标横向 release、恢复和回退验收 |
