...
| R9-SCALE-001 | P1 条件风险 | Multi-instance login/upload/model gates | 工程执行者 | R9.1 已关闭登录子项；上传准入和模型并发仍依赖进程内状态，当前继续维持单 API/单 Worker | R9.1 本提交 | 登录：Redis 原子四层预算、摘要键、服务器时钟租约、幂等完成；双实例 4/4、418/418 unit、101/101 integration。上传/模型待 R9.2/R9.3 | in_progress | H13；未关闭全部子项前不得横向扩容 |
...
| B8-STAGING-003 | P1 | B8-09/RC | 多实例登录、上传与模型闸门 | 全局请求与登录限流已共享；上传准入和模型并发仍为进程内状态 | R9.1 登录已完成；Staging 继续单 API/单 Worker，R9.2/R9.3 迁移剩余闸门 | 登录双实例并发/重启/租约/Redis 故障已通过；上传与模型证据待补 | in_progress | H-13 若要求横向扩容则阻断 |
