...
| R9-SCALE-001 | P1 条件风险 | Multi-instance login/upload/model gates | 工程执行者 | R9.1-R9.3 已关闭登录、上传和模型执行门的本地工程子项；目标拓扑仍维持单 API/单 Worker | R9.1-R9.3 本地提交 | 三类控制均使用 Redis 原子预算、摘要键、服务器时钟租约和失败关闭；模型双实例 6/6、428/428 unit、当前强制 Redis 119/119 integration | engineering_verified | H13/H14；目标服务器多实例 release、恢复与回退未执行，不得宣称横向扩容已验收 |
| R9-XLSX-STREAM-001 | P1 | Streaming XLSX shared strings | 工程执行者 | R9.1 最终回归首次出现 1 次嵌入媒体工作簿 shared-string 未解析；旧逻辑会将内部 token 字符串化 | R9.1A 本提交 | ZIP/XML 确定性预加载；残留 token 失败关闭；15/15 定向、10 轮 0 失败、4 路并发一致、419/419 unit、真实 PostgreSQL XLSX API 通过 | engineering_verified | 无；仍不宣称修复 ExcelJS 上游全部内部时序 |
| R9-CONFIRM-RECOVERY-001 | P1 条件风险 | Import final transaction timeout/retry | 工程执行者 | 默认 PostgreSQL 与远端 CI 均证明 30,196 行可能在全部暂存后卡入最终发布预算；完整性围栏又使默认 5 秒批次事务在 checkpoint 压力下误重试 | R9.3A/R9.3B + CR-005 | P2028/P2034 有界租约恢复；批次事务显式 10 秒等待/30 秒执行预算；6 秒故障注入保持单次 attempt；强制 Redis 119/119 integration；30,196/49,999 行 60.627/151.716 秒 | engineering_verified_locally | H13 目标磁盘/WAL/checkpoint/p95/p99 容量验收；live checks 以 Draft PR #4 为准 |
| P0-EXCEL-STAGING-001 | P0 | Excel unpublished isolation/integrity | 工程执行者 | 攻击测试证明旧 `pending_confirm` 记录可被通用 API 读取/修改，暂存记录和值可被篡改，来源可在预检后变化，最终 UPDATE 少一行仍可能结束成功 | CR-002 至 CR-005 | 显式 publication status；所有读取/写入路径隔离；逐记录 hash/批准 hash/来源镜像；DB 失效触发器；精确三组 affected-row 断言；失败清理与幂等重试；攻击回归和 119/119 integration | engineering_verified_locally | H10 正式职责分离、H13 目标容量和 H15 独立审查不由本地 P0 关闭 |
...
| R8-CI-001 | P1 | Layered CI and real deployment gates | 工程执行者 | 普通 CI 曾不构建实际应用镜像，Node 22/24 漂移，且前端构建上下文曾达 10.23GB | R8.1-R8.9/M8.1/R10 | Node 24.18 统一；Gate A-C 在普通 CI 执行；应用镜像、非 root/revision、Syft/Grype、PostgreSQL/E2E 均有远端证据；scheduled/manual Staging 恢复/回滚/Python 与 GPU L0 路径已建立，本机 L0 已通过 | engineering_verified / blocked_external | 目标 self-hosted Linux/GPU runner、真实 restore/RPO/RTO 和 release 仍受 H13/H14；不得写成目标环境通过 |
...
| B8-STAGING-003 | P1 | B8-09/RC | 多实例登录、上传与模型闸门 | 全局请求、登录、上传与模型执行门均已共享；目标 Compose 尚未做多实例部署 | R9.1-R9.3 已完成本地迁移；Staging 继续单 API/单 Worker，等待 H13 目标拓扑 | 三类双实例并发/重启/租约/Redis 故障均通过；模型专项 6/6，当前完整 integration 119/119 | engineering_verified | H-13/H-14 仍阻断目标横向 release、恢复和回退验收 |
...
| M2-PROMPT-REGISTRY-001 | P1 | Prompt registry/version vector | verified | M2 |
...
| M5-SELF-APPROVAL-001 | P0 | 上传者自审批与最终重鉴权 | engineering_verified | M5.1 / M5.2 |
| M5-PARTIAL-COMMIT-001 | P0 | 错误行存在时部分正式入账 | verified | M5.2 |
| M5-SNAPSHOT-COMMIT-001 | P0 | 不可变批准快照、未发布隔离与唯一 commit | engineering_verified | M5.1 / M5.2 + CR-002 至 CR-005 |
