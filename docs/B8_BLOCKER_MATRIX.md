...
| R9-XLSX-STREAM-001 | P1 待复现 | Streaming XLSX shared strings | 工程执行者 | R9.1 最终回归首次出现 1 次嵌入媒体工作簿 shared-string 未解析；隔离复跑和第二次全量均通过，尚无根因证据 | - | 必须先让未解析 token 失败关闭，再做重复/压力复现；不得把重跑绿色写成根因已修复 | open | 无 |
