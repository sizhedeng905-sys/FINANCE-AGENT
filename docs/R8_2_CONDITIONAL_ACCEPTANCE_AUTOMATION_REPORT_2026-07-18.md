...
- R8.5 修复后的完整运行已进一步通过远程 TLS、migration、全部服务健康与 API/浏览器 smoke；新的真实失败点是 backup loop 的 `mc` 配置目录不可写，导致 restore drill 没有 complete backup。R8.6 已实现非 root、新鲜度和互斥修复，完整端到端重跑仍为 `not_run`，详细证据见 `R8_6_BACKUP_RELEASE_GATE_REPORT_2026-07-18.md`。
