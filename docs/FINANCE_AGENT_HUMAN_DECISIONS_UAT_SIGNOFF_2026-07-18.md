...
- 冲突结论：Q01 勾选“整张表整体是一笔汇总记录”，Q04 后补充却要求 Excel 按明细入账。H01 在项目负责人明确最终粒度前继续人工选择、禁止自动判断。
...
- 状态：`Pending - conflicting owner answers`
- 决策负责人：财务负责人
- 需要共同确认：业务负责人
- 当前系统保守行为：每个手工请求、Excel 映射行、OCR 确认任务或完成工单生成一条记录；模板决定 `actual/reconciliation/budget`，系统不自动判断明细或汇总，也不自动阻断跨文件双计。
- 已收到的决定草案：
  - 汇总与明细必须二选一，不允许同时计入经营统计。
  - Q01 选择“整表一笔汇总”，但自由文本要求“按明细入账、总额由程序累加”，两者冲突。
  - 冲突解决前不自动推断粒度；财务必须显式选择，正式提交必须保存粒度和互斥校验版本。
- 仍需填写：
  - 最终选择：每行一笔明细，还是整表一笔汇总；
  - 汇总行稳定识别特征和互斥键；
  - 允许的例外、历史数据规则和验收样例。
- 决策人：
- 岗位/角色：
- 决策日期：
- 证据引用：负责人问卷 Q01/Q02 及 Q04 后补充
- 冲突与例外：Q01 与自由文本冲突，未关闭
