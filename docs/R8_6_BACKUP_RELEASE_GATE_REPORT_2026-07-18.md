...
R8.6 已完成永久修复与定向验收，但本报告不把修复后的完整 release/rollback 写成 passed；必须在干净提交后重跑。
...
- R8.6 commit 的完整 release、新鲜备份、restore drill、运行日志、manifest 和 rollback：`not_run`。
...
提交 R8.6 后执行完整 `npm run staging:release`，解析经 seal 校验的 manifest，运行日志门禁，再用同一 manifest 执行 rollback 和二次 smoke。最后执行 `down -v --remove-orphans` 并断言本项目容器、网络和卷残留均为 0。
