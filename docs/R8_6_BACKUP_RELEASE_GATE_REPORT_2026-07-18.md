...
- rollback 暴露的 Prisma/OpenSSL runtime warning 已由 R8.7 定向修复，R8.7 完整 release 仍为 `not_run`。
...
提交 R8.7 后重跑完整 release，确认 migration/rollback 不再出现 Prisma OpenSSL 检测告警，复核运行日志，然后执行资源清理与零残留断言。
