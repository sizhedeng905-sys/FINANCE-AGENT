...
- 增加 Redis 共享全局固定窗口限流、生产 fail-closed 连接、Worker 心跳和 readiness 依赖；R9.1-R9.3 随后将登录、上传准入与模型 FIFO 执行门升级为 Redis 原子共享控制。Compose 仍固定单 API、单 Worker，等待 H13/H14 目标多实例部署与恢复验收。
...
- 登录、上传准入与模型执行门已完成本地多实例共享控制和故障测试。若 H-13 要求 API/Worker 横向扩容，仍必须在指定目标服务器完成真实多实例 release、模型故障、恢复和回退测试；本地证据不等同于目标部署通过。
