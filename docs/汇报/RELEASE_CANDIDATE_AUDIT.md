...
| RC-H13-01 | P1 条件风险 | 登录、上传与模型闸门均已共享化，但目标多实例部署仍未验收 | 当前 Compose 锁定单 API/单 Worker | R9.1-R9.3 本地双实例故障测试通过；扩容前仍需 H13/H14 目标 release、恢复与回退 | `engineering_verified / blocked_external` |
...
本段 RC 结论只描述 `4d597721` 当时已经检查的范围，已被 2026-07-18 至 2026-07-21 的 R 系列重新审计取代。R1-R8 的本地可执行代码类 P0/P1 已按分层门禁关闭，R9.1-R9.3B 的共享控制、恢复和发布事务收敛也已获本地工程证据；目标 Staging、真实模型/财务真值、新 head CI 和 H01-H16 仍是外部或人工发布门禁。不得把“代码类 P0/P1 已关闭”改写成“项目无发布阻塞”；实时状态以 `docs/汇报/B8_BLOCKER_MATRIX.md` 为准。
