...
当前批次：R0-R11 真实性、安全、恢复与并发重新审计继续收口。CR-002 至 CR-010 已关闭 Excel staging、Prompt provenance、治理、文档、production-safe system registry 和运行镜像供应链问题。CR-011 新增周五演示 E2E，以三行合成 Excel 证明不同财务审批、批准前发布隔离、3 条逐分记录、项目 structure、老板报告、幂等重放和 Snapshot 哈希；CR-012 又建立只允许本机精确测试库的 reset/verify/start/test 与完整现场交付包，两者已随 SHA `66749b3` 推送并取得 Build/CodeQL 双绿。CR-013 已把真实 Excel AI 建议接入财务人工草稿，跨模板、503、任务切换和登出均失败关闭，保存前 mappings 写请求与正式记录均为 0。当前候选通过 50 suites / 464 unit、14 suites / 124 PostgreSQL/Redis integration、20/20 Playwright、43 migrations 双路径和双端 build，另有 demo 配置 6/6 与一键故事 1/1。CR-013 远端 CI 与后端审核决定/provenance 持久化仍待后续；三次人工演练为 `NOT_RUN`，真实口径/准确率为 `REAL_SAMPLE_NEEDED`，目标 Staging 为 `EXTERNAL_RESOURCE_NEEDED`。
...
| R11 最终交接 | 进行中 | CR-010 至 CR-012 已推送且远端全绿；CR-013 Excel AI 本页草稿桥接 20/20 E2E 通过；下一子块是服务端审核决定/provenance；三次人工演练未执行，保持 Draft、不 merge、不转 Ready |
...
H01/H03 等明确规则为 `OWNER_CONFIRMED`；未回答细节采用 `SAFE_DEFAULT_ACTIVE`；H04-H06/H09 的真实真值为 `REAL_SAMPLE_NEEDED`；H12-H15 的目标资源为 `EXTERNAL_RESOURCE_NEEDED`。工程实现继续失败关闭，不能把 owner decision、合成测试或本地部署写成真实业务/生产验收已完成。
...
| B8-03 大批量 Excel 确认 | 工程完成 / 目标容量待验 | 短事务 Worker、lease/恢复、显式未发布隔离、内容完整性与精确发布围栏通过；5,001/30,196/49,999 行本地闭环通过，目标 p95/p99 仍待 H13 |
...
| B8-06 权限、Cookie、文件与数据安全 | `ENGINEERING_VERIFIED` | AI 日志所有权、独立 admin/auditor、生产 Cookie/JWT、主动内容、资源上限和 Git/DLP 门禁通过；H10/H11 未决细节采用 `SAFE_DEFAULT_ACTIVE` |
| B8-07 模型控制面、GPU 与反向代理 | 工程完成 | 不可变部署快照、认证身份探针、跨进程 GPU 状态机、容器/SBOM/CVE 和 50 MiB 代理边界通过 |
| B8-08 人工财务 UAT | `REAL_SAMPLE_NEEDED / OWNER_UAT_VERIFIED pending` | 八场景匿名 manifest、逐分对账和 `_test` 数据库门禁完成；仍需真实真值与项目负责人亲自 UAT，不需要多角色签字 |
...
| RC-00 至 RC-04 | `ENGINEERING_VERIFIED / EXTERNAL_RESOURCE_NEEDED` | 攻击性审计、确定性修复、迁移双路径、全量门禁、PR reviewer guide 和交接包完成；发布仍受真实 Staging、真实真值、独立审计和 owner UAT 阻断 |
...
- R8.2 随后两次构建被 Docker Hub BuildKit SBOM scanner 认证端点超时阻断，当时记录为外部阻断且不再重试该条件。审计证明 build-time SBOM 未被 release 消费或封存；现移除 mutable scanner 网络耦合，保留 max provenance，并以固定版本/哈希的 Syft SPDX、固定 Grype、sealed supply-chain index 作为正式 SBOM/CVE 门禁。
...
- R8.6 干净提交的完整 release 运行 1010.9 秒，sealed manifest `20260718T221820Z-97efc1856f28` 的 config、image identity、SBOM、CVE、migration、smoke、restore drill 七项 gate 全部 passed。升级前/发布后备份均成功，隔离恢复与 fault injection 通过；同 manifest rollback 55.6 秒完成保护性备份、镜像与 migration 复核、四角色登录、readiness/worker、metrics 和二次 smoke，`dataRestored=false`。没有更早合法 manifest，因此跨版本回退仍未验证；详见 `docs/汇报/R8_6_BACKUP_RELEASE_GATE_REPORT_2026-07-18.md`。
...
- R8.9 的 Syft 替代链已由 GitHub run `29752263099` 真实执行：Linux runner 能生成 SPDX、调用固定 Grype，数据库/build/unit/E2E 均在扫描前完成。该 run 的失败不再是 entitlement，而是扫描正确发现旧 Nginx 的可修复 Critical，因此 R8.9 工具链已验证，漏洞转入独立 M8.1 修复。详见 `docs/汇报/R8_9_CI_SBOM_ENTITLEMENT_HARDENING_REPORT_2026-07-20.md`。
- M8.1 先实测 `nginx:1.28.3-alpine`，当前 Grype 数据库仍发现 OpenSSL Critical 修复版本缺口；最终固定到官方稳定 `nginx:1.30.4-alpine3.24@sha256:97d490...e5b46`。本地真实前端和 R5 夹具各生成 72 包 SPDX，`--only-fixed --fail-on critical` 均为 0；远端 Build run `29755386892` 的应用供应链 job 2m37s、PostgreSQL/E2E/R5 job 12m30s 均成功，CodeQL run `29755387035` 成功。详见 `docs/汇报/M8_1_NGINX_CI_SECURITY_REFRESH_2026-07-20.md`。
- M3.2 将 Excel AI 分类/映射接入现有 `ImportTask`、Prompt Registry、Provider 和 AI 审计链：只发送有预算的列摘要，严格限制当前项目模板、字段、证据与转换，所有输出固定为 `NEEDS_FINANCE_REVIEW`。调用使用内容寻址、UUID 租约、advisory lock、3 次重试预算和发送前策略复核；迟到响应、模板在分类中停用、Profile 哈希篡改、kill switch 与恶意 JSON 均失败关闭且不写正式记录。后端 46/46 suites、401/401 tests，PostgreSQL 分组全量 9/9 suites、92/92 tests，34 条 migration 空库及 33→34 升级、前后端 build、前端 runtime 4/4 和 680 文件卫生通过。真实模型准确率与 M4-M8 仍未宣称完成；详见 `docs/汇报/M3_2_EXCEL_AI_SUGGESTION_REPORT_2026-07-20.md`。
- M4 复用现有 OCR task/attempt/correction、OCR IR 和 AI 调用台账，完成有界分类/映射建议、source/evidence 白名单、跨页冲突保留、人工 `reviewRevision`、内容寻址 ValidationSnapshot 与 bbox 复核 UI。固定本地 PDF.js Worker 从鉴权接口读取原件，响应为 attachment/octet-stream 时仍依据锁定文件元数据解析；未知旋转不画误导框。真实 API Playwright 覆盖 AI 零入账、PDF 画布/bbox、390px 布局、修订使旧校验失效和重新校验后确认；M5 的直接 API 最终重鉴权、自审批和事务批准仍为开放 P0。详见 `docs/汇报/M4_OCR_AI_EVIDENCE_REVIEW_REPORT_2026-07-20.md`。
- M5.1 收紧 OCR 正式批准：命令必须携带 expected task/review/validation/payload hash 和逐项 warning ID；最终事务重新读取账号/角色、拒绝上传者自审批、重验来源安全状态/IR/模板/候选/证据，并冻结完整批准快照。两个财务并发只写一次，同键重放返回原结果，改体重放 409；空库 36 条与 35→36 migration、46/46 suites 403/403 tests、PostgreSQL 攻击场景和 Playwright 17/17 通过。Excel 的 partial commit 仍保持 P0 open，详见 `docs/汇报/M5_1_OCR_APPROVAL_COMMIT_REPORT_2026-07-20.md`。
- M5.2 当时按 H01 每行明细关闭了 Excel partial commit：普通错误明细不可排除，疑似汇总行必须财务处置，任何修改使旧校验失效。该阶段 403 unit、96 PostgreSQL、17 Playwright 及 36→37 migration 是历史证据，不覆盖后来发现的通用 API 暂存隔离与逐记录完整性问题；现行结论见 `docs/汇报/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md`。
- M6 完成 canonical ReportSnapshot：固定查询只读取 `confirmed + actual`，使用 repeatable-read、Decimal、分币种和来源版本/hash；AI 只能逐字选择服务端 Claim 白名单，值篡改、额外数字、虚构实体、原因/比较和 warning 遗漏均失败关闭。H06/H08 真实口径为 `REAL_SAMPLE_NEEDED`，报告复核角色为 `OWNER_CONFIRMATION_NEEDED`；详见 `docs/汇报/M6_REPORT_SNAPSHOT_GROUNDING_REPORT_2026-07-20.md`。
- M7 红测复现六个相同 ReportSnapshot 并发请求出现部分 409，现仅对 `P2002/P2034` 做最多三次新事务重试并复用唯一快照。攻击回归覆盖报告权限、kill switch、并发单 Provider、超时脱敏、截断 JSON、资源边界、Worker 恢复和 Staging 静态门禁；47/47 unit、97/97 PostgreSQL、17/17 Playwright、41 条 migration 双路径、构建和两套 0 vulnerability 审计通过。49,999 行两次全量采样为 46.014 秒和 143.199 秒，仍保留性能抖动风险，详见 `docs/汇报/M7_ATTACK_RESOURCE_PROVIDER_ACCEPTANCE_2026-07-20.md`。
- M8 完成最终证据收口并以 `30c6ead` 推送到 Draft PR #4；Prompt manifest/guard 4/4 unit 与 3/3 PostgreSQL、前后端 build、runtime 4/4、41 条 migration 双路径、708 文件卫生和两套 0 vulnerability 审计通过。受保护 Prompt Catalog 和 H 门禁仍未关闭，详见 `docs/汇报/M8_FINAL_EVIDENCE_AND_DRAFT_PR_HANDOFF_2026-07-20.md`。
- 生产全局请求限流、登录口令限流、上传准入与模型执行门已由 Redis 共享。R9.1/R9.2 使用原子 Lua、摘要身份键、Redis 时钟租约、续租/崩溃回收和断连失败关闭；R9.3 将 AI、OCR 与推理健康探针统一置于共享 FIFO 并发预算下，租约丢失会中止 Provider 请求且不会污染熔断统计。模型双实例专项 6/6、后端 47 suites / 428 tests、当前 PostgreSQL/Redis 13 suites / 119 tests 与 build 本地通过。代码层子项已关闭，但 B8-09 Compose 继续固定单 API、单 Worker，目标多实例部署、恢复和回退受 H13/H14 阻断；详见 R9.1-R9.3 三份验收报告。
- R9.1A 对一次 shared-string 未解析观察实施确定性预加载和残留 token 失败关闭。Excel parser 15/15、连续 10 轮 0 失败、单轮 4 路并发一致、全量 unit 419/419、PostgreSQL 真实 XLSX API 场景与 41 migrations 通过；详见 `docs/汇报/R9_1A_XLSX_SHARED_STRING_HARDENING_REPORT_2026-07-21.md`。
...
- R9.3B 根据远端 run `29768468874` 的 30,196 行超时红灯收敛了计数与预检事务；当时 114 个测试和 25.502/42.954 秒为历史性能证据，并未覆盖通用 API 暂存隔离、逐记录篡改和实际少更新一行。CR-002 至 CR-005 随后重新打开并关闭这些 P0，当前强制 Redis 全量为 119 tests，完整围栏下 30,196/49,999 行为 60.627/151.716 秒；目标容量仍受 H13 阻断。
- CR-006 先以 HTTP body 单测证明旧路径没有执行版本化用户模板，再以老板助手 PostgreSQL 用例暴露 Prisma Decimal/Date 不能直接 canonical 化。现统一使用 `ai-prompt-json/1.0` 将受控服务端 DTO 规范化，`inputJsonSha256` 与实际渲染变量一致；执行 provenance 1.1 与调用向量 1.1 冻结 Prompt/component/system/template/rendered/schema/input、Provider/模型和输出哈希。未知/重复模板变量、任意类实例、非有限数、循环、原型污染、Schema 漂移和预算超限失败关闭；调用日志只保存哈希和字段摘要。48 suites / 435 unit、老板助手 1/1、Excel/OCR AI 5/5、报告 1/1、构建、runtime、732 文件卫生和两套依赖审计通过。该提交当时尚未完成的 production bootstrap 已由后续 CR-009 关闭；受保护 Prompt 文档仍不由 Codex 伪造。
- CR-009 将代码内已审计 Prompt manifest 与受控模型路由落到现有 `AiPromptVersion/ModelDeployment/TaskModelRoute`，不再依赖会写入演示业务数据的通用 seed。内置 development/mock profile 与严格 custom JSON 只保存 secret reference；初始化持有 PostgreSQL advisory transaction lock 并对并发序列化冲突做有界事务重试，配置/hash 漂移、未知启用项、缺少启用 secret 和禁用 deployment 上的启用 route 均失败关闭。独立空库 acceptance 验证 43 migrations、两个并发进程精确收敛为 changed/unchanged、11/1/7 系统计数、11 类业务计数全 0、Mock 真调用、API/Worker 启动以及故意漂移拒绝。全量证据为 50/50 suites、464/464 unit，14/14 suites、124/124 PostgreSQL/Redis integration，17/17 Playwright、双端 build、runtime 4/4 和两套 0 vulnerability 审计；详见 `docs/汇报/CR_009_PRODUCTION_SAFE_AI_SYSTEM_REGISTRY_REPORT_2026-07-21.md`。
- CR-010 修复 CR-009 新 SHA 唯一的容器供应链红灯。失败 SBOM 将 fixable Critical 精确定位到 Node 基础镜像全局 npm 的 `tar/undici/brace-expansion`；最终镜像现移除 npm/npx/Corepack，Staging migration 直接调用项目内 Prisma，CI 同时断言 entrypoint、非 root、Node/OpenSSL/Prisma 和编译入口。新 SBOM 为 433 components、0 npm CLI components；固定 Grype fresh DB 扫描、50/464 unit、14/124 PostgreSQL/Redis、17/17 Playwright、migration/build/runtime/audit 均本地通过。SHA `1abe513` 的 Build run `29823851399` 与 CodeQL run `29823851377` 均成功，详见 `docs/汇报/CR_010_RUNTIME_IMAGE_SUPPLY_CHAIN_RECOVERY_REPORT_2026-07-21.md`。
- CR-011 用运行时生成的三行合成 Excel 串起真实 UI/API：公式缓存只产生 warning，财务 A 不能自批，财务 B 重新校验后批准；批准前 records、project structure 与老板金额不变，未来 record ID 的通用读写接口均 404；批准后 3 条 confirmed Excel 记录逐分合计 `13422.21`，重复批准返回同一结果，项目/老板/Snapshot 增量一致，`sourceDigest/snapshotHash` 均重算匹配。50/464 unit、14/124 PostgreSQL/Redis、18/18 Playwright、migration/build/runtime/audit 本地通过，详见 `docs/汇报/CR_011_FRIDAY_EXCEL_REPORT_DEMO_E2E_2026-07-21.md`。
- CR-012 将上述 E2E 固定为周五现场包：`demo:*` 命令拒绝 production、远程或非 `finance_agent_test` 数据库，清除外部 Provider/S3 凭据，只启动本地 Mock API/Web；6/6 配置负测、reset/verify、API/Web smoke 和一键真实 API E2E 1/1 通过。三次人工演练保持 `NOT_RUN`，详见 `docs/汇报/CR_012_REPEATABLE_FRIDAY_DEMO_DELIVERY_2026-07-21.md`。
- CR-013 已接通真实 Excel AI 建议 POST/GET 和财务本页草稿，显示模板、证据、warning、Provider/模型/Prompt 与哈希，逐列采纳/拒绝/修改/忽略均不自动保存或入账；跨模板、503、任务切换和登出失败关闭。后端 50/464、专项 E2E 2/2、完整 Playwright 20/20 和双端 build 通过。审核决定服务端持久化尚未完成，详见 `docs/汇报/CR_013_EXCEL_AI_ADVISORY_DRAFT_BRIDGE_2026-07-21.md`。
...
- 详细步骤与证据见 `docs/B8_09_STAGING_RUNBOOK.md`、`docs/汇报/B8_09_STAGING_REPORT.md` 和 `docs/汇报/RELEASE_CANDIDATE_AUDIT.md`。H12-H16 当前分别归入 `OWNER_CONFIRMATION_NEEDED`、`REAL_SAMPLE_NEEDED`、`EXTERNAL_RESOURCE_NEEDED` 或 owner UAT 门禁。

B8-08 已完成的工程证据：

- UAT-01 至 UAT-08 覆盖 Excel、劳务、报销/OCR、报表、老板 AI、重复业务和冲销/更正；运行手册明确每项人工负责人和回退阶段。
- `uat:init` 只在 Git 忽略目录生成匿名 manifest、问题台账和历史验收模板，重复执行不覆盖人工内容；`uat:validate` 固定八场景。旧四角色签字字段仅为兼容产物，不再是项目治理门禁。
...
- 空白人工输入烟测明确返回 `automatic=awaiting_input`、`human=external_unverified`；脚本不能把自动结果伪装成真实样本或 owner UAT。
- 详细工具与状态见 `docs/B8_08_FINANCE_UAT_RUNBOOK.md` 和 `docs/汇报/B8_08_FINANCE_UAT_REPORT.md`。
- 自动化结果：24/24 migrations；24/24 Jest suites、240/240 tests；2/2 PostgreSQL suites、59/59 tests；14/14 Playwright；前后端 build 通过。
- H01-H12 和 H16 按当前台账分为 owner 决定、真实样本和 owner UAT；未关闭项继续失败关闭，当前项目不声明财务验收通过或生产就绪。
...
- 详细证据见 `docs/汇报/B8_07_MODEL_CONTROL_PLANE_REPORT.md`。真实 Staging/TLS、集中监控和备份恢复属于 B8-09，当前仍不声明生产就绪。
...
- 详细证据见 `docs/汇报/B8_06_SECURITY_HARDENING_REPORT.md`。H10/H11 的 step-up 与扫描 bypass 仍为 `OWNER_CONFIRMATION_NEEDED`。
...
- 详细证据见 `docs/汇报/B8_05_AI_CLAIM_GROUNDING_REPORT.md`。H08 标准答案为 `REAL_SAMPLE_NEEDED`，H12 Provider 清单为 `OWNER_CONFIRMATION_NEEDED / EXTERNAL_RESOURCE_NEEDED`。
...
- 详细证据见 `docs/汇报/B8_04_OCR_ASYNC_PRECISION_REPORT.md`。H-04/H-05 仍需人工完成 17 份真值与盲测冻结，本阶段不声明真实准确率。
...
- 详细证据见 `docs/汇报/B8_03_LARGE_EXCEL_CONFIRMATION_REPORT.md`。H-03 跨来源业务指纹仍等待财务和业务负责人输入。
...
- 上述结论只代表工程自动化门禁通过，不代替负责人对冲销规则的决定、真实会计真值和 owner UAT。

## 真实业务数据 B0/B1/B2 结论

2026-07-14 已对 Git 忽略的本地 `数据文件/` 完成只读 B0 扫描：112/112 个物理文件均建立匿名分类，扫描前后 SHA-256 一致，公开报告不含原始路径、文件名、完整哈希或业务值。聚合报告见 `docs/汇报/REAL_BUSINESS_DATA_TEST_REPORT.md`，原始映射只保存在被忽略的 `.realdata-test/`。
...
当前自动化证据：18/18 Prisma migrations；17/17 Jest suites、184/184 tests；30/30 真实 PostgreSQL；14/14 Playwright；前后端 build。真实 Qwen/Paddle 运行稳定性已验收；OCR 字段准确率和 L3 会计真值仍为 `REAL_SAMPLE_NEEDED`。
...
- B4：18 个 migration 已部署到测试库；Decimal、日期边界、草稿/确认/作废、并发确认、audit、ledger 与四来源快照通过 29/29 PostgreSQL 集成测试。L3 会计真值仍为 `REAL_SAMPLE_NEEDED`。
...
- B7：前后端 production build、17/17 Jest suites、184/184 tests、30/30 PostgreSQL integration、14/14 Playwright、Prisma、hygiene、依赖审计和 112 份原件复扫全部通过。历史交付物见 `docs/汇报/B7_FINANCE_UAT_ACCEPTANCE.md`；工程状态为 `ENGINEERING_VERIFIED`，财务/OCR 真值为 `REAL_SAMPLE_NEEDED`。
...
| I 真实业务数据 | B0-B7 工程完成 | 文件/Excel/OCR、经营记录、72 条 AI 基准、故障恢复、并发、真实模型切换和全量回归已通过；OCR 标签与 L3 财务真值为 `REAL_SAMPLE_NEEDED` |
...
详细迁移状态见 `docs/汇报/API_MIGRATION_MATRIX.md`。
...
CR-009 已复核，CR-010 至 CR-012 已推送并取得同 SHA Build/CodeQL 绿色证据，CR-013 Excel AI 财务草稿桥接已本地通过。下一检查点是提交并验证 CR-013 远端 CI，然后独立实现 expected version/review revision、人工决定和建议 provenance 的服务端持久化；项目负责人仍需按交付包完成三次人工演练。真实样例/真值为 `REAL_SAMPLE_NEEDED`，目标 Linux release/restore/RPO/RTO/rollback 为 `EXTERNAL_RESOURCE_NEEDED`。PR #4 保持 Draft，不 merge、不标记 Ready。
...
- `docs/ARCHITECTURE.md`、`docs/SECURITY.md`、`docs/LOCAL_SETUP.md` 与 `docs/计划/PR_PREPARATION.md` 补齐架构、安全、Windows/跨平台初始化和 PR/回滚说明。
...
- 默认开发配置仍使用 Mock AI/OCR；真实 GPU 性能与企业文件适配已验证，模型准确率只在脱敏样本与业务真值验证后声明。
