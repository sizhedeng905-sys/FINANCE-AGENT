...
- Staging、模型网络和发布证据：CR-036 已参数化目标边界；CR-037 已阻断 target 复用本地 CA/identity/seed/域名/初始化和 mutable image；CR-038 已增加 17 项只读、匿名目标预检及普通 CI 合成门禁。真实目标运行结果仍受 H13-H16 门禁约束，本机 Demo 保持不变。
