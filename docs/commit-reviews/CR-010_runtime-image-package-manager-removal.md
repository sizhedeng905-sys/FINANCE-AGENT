...
- CR-009 远端 Build run `29821449158`：PostgreSQL/E2E job 成功，容器 job 仅在上述 fixable-Critical gate 失败；CodeQL run `29821448996` 成功。CR-010 新 SHA 的远端结果在 push 后更新 Draft PR 和后续持续执行报告。
...
`LOCAL_ENGINEERING_VERIFIED / REMOTE_CI_PENDING`。本地构建、测试、镜像结构、SBOM 和 fixable Critical 扫描已通过；只有新 SHA 的 Build 与 CodeQL 均成功后，供应链红灯才可标记为远端关闭。
