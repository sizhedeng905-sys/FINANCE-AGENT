...
export class LocalPaddleOcrProvider implements OcrProvider {
  ...
  async recognize(input: OcrProviderInput): Promise<OcrProviderResult> {
    ...
    const payload = this.outputValidator.validate(localPaddleResponseSchema, await response.json()) as OcrProviderResult;
...
const localPaddleResponseSchema = {
  ...
  properties: {
    ...
    pages: { type: 'array', items: { type: 'object', additionalProperties: true }, nullable: true },
    textBlocks: { type: 'array', items: { type: 'object', additionalProperties: true }, nullable: true },
    tables: { type: 'array', items: { type: 'object', additionalProperties: true }, nullable: true },
    fieldCandidates: {
      type: 'array',
      items: {
        type: 'object',
        properties: {
          targetFieldId: { type: 'string', nullable: true },
          targetFieldKey: { type: 'string', nullable: true },
    ...
    rawResultRef: { type: 'string', nullable: true }
  },
