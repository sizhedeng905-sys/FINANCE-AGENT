...
export function transformKeyForFieldType(fieldType: string): ImportTransformKey {
  if (fieldType === 'money' || fieldType === 'number') return 'DECIMAL_CANONICAL_V1';
  if (fieldType === 'date') return 'DATE_ISO_WITH_LOCALE_V1';
  if (fieldType === 'select') return 'ENUM_ALIAS_LOOKUP_V1';
  if (fieldType === 'text' || fieldType === 'textarea') return 'TRIM_TEXT_V1';
  return 'IDENTITY_V1';
}
