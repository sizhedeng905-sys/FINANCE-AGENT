...
| R0-R10 修复与再验收 | `engineering_verified / blocked_external` | R8.9 分层 CI、M8.1 Nginx Critical、R9 共享控制、Excel 暂存完整性重审和 R10 本地模型 L0 均有工程证据；L1 真值与目标 release 仍受 H04-H16 阻断 |
| R11 最终交接 | `in_progress` | Excel P0 已收口；P1-1 Prompt 真执行与 provenance 已本地通过，production-safe bootstrap 仍在推进；不 merge、不转 Ready |
...
| 发布结论 | `blocked` | 目标环境 P0/P1、真实 Staging、恢复演练、安全复核、财务/OCR/AI 真值和最终签字均未完成 |
...
- R9.3B 针对远端 run `29768468874` 中“30,196 行已全部暂存但 180 秒内未完成发布”的红灯，把重复全表计数改为租约围栏增量，并将全量哈希重算移到带心跳的发布预检。该阶段的 114 个测试是历史性能证据，尚未覆盖通用 API 提前可见、暂存内容篡改和最终少更新一行；这些缺口后来由 CR-002 至 CR-005 重新打开并关闭。详见 [`docs/R9_3B_IMPORT_PUBLICATION_TRANSACTION_HARDENING_REPORT_2026-07-21.md`](docs/R9_3B_IMPORT_PUBLICATION_TRANSACTION_HARDENING_REPORT_2026-07-21.md)。
- Excel 暂存完整性重审先用攻击测试证明未发布记录可被列表/详情/修改/确认/作废路径触达，随后引入显式 `unpublished|published` 可见性、逐记录 canonical SHA-256、来源镜像、数据库失效触发器、最终精确 affected-row 断言和失败后全量清理。强制 Redis 全量现为 13/13 suites、119/119 tests；30,196/49,999 行确认分别为 60.627/151.716 秒，仍低于 180 秒产品断言，但 49,999 行余量不足以替代 H13 目标容量验收。完整证据见 [`docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md`](docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md)。
...
M5.2 当时关闭了 Excel `valid_rows_only` 部分发布问题：每个有效明细行独立入账，普通错误明细不可通过排除绕过，疑似汇总行必须财务明确处置，任何修改使旧校验失效。第 37 条 migration、403 个 unit、96 个 PostgreSQL 和 17 个 Playwright 是该阶段的历史证据；它们不覆盖后来发现的通用 API 暂存隔离与逐记录完整性问题。现行 P0 结论以 CR-002 至 CR-005 和 [`docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md`](docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md) 为准。
...
| Excel 财务批准事务 | `engineering_passed` | review revision、第二财务、显式未发布隔离、逐记录内容哈希、数据库失效触发器、最终重鉴权和精确更新行数围栏；普通错误明细不能绕过，正式 H01/H10 签字仍待完成 |
...
| [`docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md`](docs/P0_EXCEL_STAGING_INTEGRITY_CLOSURE_2026-07-21.md) | Excel 未发布隔离、内容完整性、原子发布攻击回归与当前 P0 结论 |
