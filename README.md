...
| AI 映射补充 M0-M8 | `M0-M3_passed / M4_in_progress` | Excel 列摘要与 OCR 有界证据均只生成建议；OCR 已有 source/evidence 绑定、跨页冲突、review revision 与确定性校验快照，bbox UI 尚在实施 |
...
| OCR AI 分类/映射 | `engineering_partial` | M4.1/M4.2 已验证受限证据摘要、source/evidence 绑定、跨页冲突、review revision、旧快照失效和重新校验；bbox UI 待完成 |
