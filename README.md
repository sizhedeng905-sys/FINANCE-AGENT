...
当前仓库已经从前端原型推进到 React 前端、NestJS 后端、PostgreSQL 数据库、异步 Excel/OCR、结构化 AI Claim、本地模型控制面和 Staging 工程。R1-R8.9 已完成前端真实性、日志、容量、恢复、镜像身份、财务并发/精度/幂等、数据生命周期、step-up 和分层 CI 的工程修复。M0-M8 已完成非生产工程与合成验收：Excel/OCR 只生成受控建议，人工修订会使旧校验失效，正式批准要求另一名财务、当前校验哈希和不可变批准快照；Excel 按 H01 每个有效明细行生成一条记录，任何阻断错误都会使整批不发布；报告数字只来自 canonical ReportSnapshot、Decimal 和固定查询。M8 提交 `30c6ead` 已推送至 Draft PR #4，CodeQL run `29752262976` 通过；Build run `29752263099` 的业务构建、410 个后端单测、97 个 PostgreSQL 集成测试和 17 个 Playwright 用例均通过，但供应链门禁正确检出旧 Nginx 镜像的可修复 Critical 漏洞。M8.1 已在本地将全部 Nginx 消费者统一到实扫通过的官方稳定镜像 `nginx:1.30.4-alpine3.24@sha256:97d490...e5b46`，并保持 Critical 阈值不变，待本提交推送后由远端 CI 复验。受保护 Prompt Catalog 仍为空；目标 Linux Staging、正式职责分离、财务/OCR/AI 真值和人工签字也未完成，因此本项目**不是 production-ready**。
...
| R0-R11 修复与再验收 | `in_progress / blocked_external` | R8.9 已在 GitHub 真正执行分层业务门禁；M8.1 Nginx Critical 修复本地实扫通过、等待远端复验；目标 release 仍受 H13/H14 阻断；retention 仅 dry-run、step-up 默认关闭 |
...
- 当前 Build and acceptance：[run 29752263099](https://github.com/sizhedeng905-sys/FINANCE-AGENT/actions/runs/29752263099)，业务门禁全部通过，旧 Nginx 的可修复 Critical 门禁失败；M8.1 已本地修复，等待新 run
- 当前 CodeQL：[run 29752262976](https://github.com/sizhedeng905-sys/FINANCE-AGENT/actions/runs/29752262976)，成功
...
- M8.1 没有降低 `--only-fixed --fail-on critical`：第一次候选 `1.28.3-alpine` 仍被当前漏洞库拒绝，最终选择 2026-07-18 更新的官方稳定版 `1.30.4-alpine3.24`。真实前端镜像与 R5 夹具均由 Syft 1.44.0 识别 72 个包、Grype 可修复 Critical 为 0；Nginx 19/50 MiB 接受、50 MiB + 1 拒绝和无临时残留也通过。详见 [`docs/M8_1_NGINX_CI_SECURITY_REFRESH_2026-07-20.md`](docs/M8_1_NGINX_CI_SECURITY_REFRESH_2026-07-20.md)。
...
以下是截至 2026-07-20 的最近工程基线；M8.1 远端复验将在本提交推送后生成：
...
| M8.1 Nginx Critical 修复 | `engineering_verified_locally / awaiting_remote_ci` | 官方稳定镜像固定 tag+digest；真实前端与 R5 夹具各 72 包、可修复 Critical 0；供应链攻击 17/17、上传边界和非 root 行为通过 |
...
| 最新 GitHub Build | `failed_security_gate / fixed_locally_awaiting_rerun` | run `29752263099` 的应用业务门禁全绿，前端和 R5 镜像因旧 Nginx 可修复 Critical 失败；M8.1 本地等价门禁已通过 |
| 最新 GitHub CodeQL | `passed` | run `29752262976`；JavaScript/TypeScript 分析成功 |
