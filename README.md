...
当前仓库已经从前端原型推进到 React 前端、NestJS 后端、PostgreSQL 数据库、异步 Excel/OCR、结构化 AI Claim、本地模型控制面和 Staging 工程。CR-002 至 CR-010 已关闭 Excel staging、Prompt provenance、治理、production-safe AI 系统登记和后端运行镜像供应链问题。CR-011 建立周五演示级“Excel 到经营报告”合成 E2E；CR-012 又把它整理为只允许本机 `finance_agent_test`、可 reset/verify/start/test 的[周五演示交付包](docs/deliveries/2026-07-24/README.md)，两者已随 SHA `66749b3` 推送并取得 Build/CodeQL 双绿。CR-013 把真实 `POST/GET /import-tasks/:id/ai-suggestions` 接入财务映射页，并已随 SHA `7d363f6` 取得 Build/CodeQL 双绿。CR-014 在此基础上由服务端核验和持久化四类人工决定及完整 provenance，并已随 SHA `5580ce3` 取得 Build/CodeQL 双绿。CR-015 在第二财务确认页分页展示上述证据并在读取失败时暂停批准；CR-016 又把批准完成后的页面定位到本批正式记录，并提供 `excel-approval/1.0` 不可变快照与记录列表双向跳转。当前候选通过 50 suites / 464 个后端单测、完整 PostgreSQL/Redis 回归 11 suites / 111 个实际执行测试（仓库定义的 3 suites / 14 tests 按既有条件跳过）、完整 Playwright 21/21、44 条 migration 双路径和前后端构建；demo 配置 6/6 与一键故事 1/1 也通过。M0-M8 的非生产工程与合成验收保持有效：AI 只生成受控建议，财务批准前没有正式可见记录，报告数字只来自 canonical ReportSnapshot、Decimal 和固定查询。CR-015 本地提交 `2a59509` 与 CR-016 因三次无法连接 `github.com:443` 尚待推送和同 SHA 远端验证；三次人工演练仍为 `NOT_RUN`，目标 Linux Staging 为 `EXTERNAL_RESOURCE_NEEDED`，财务/OCR/AI 真值为 `REAL_SAMPLE_NEEDED`，项目负责人 UAT 尚未达到 `OWNER_UAT_VERIFIED`，因此本项目**不是 production-ready**。
