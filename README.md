...
当前仓库已经从前端原型推进到 React 前端、NestJS 后端、PostgreSQL 数据库、异步 Excel/OCR、结构化 AI Claim、本地模型控制面和 Staging 工程。CR-002 至 CR-010 已关闭 Excel staging、Prompt provenance、治理、production-safe AI 系统登记和后端运行镜像供应链问题。CR-011 建立周五演示级“Excel 到经营报告”合成 E2E；CR-012 又把它整理为只允许本机 `finance_agent_test`、可 reset/verify/start/test 的[周五演示交付包](docs/deliveries/2026-07-24/README.md)，两者已随 SHA `66749b3` 推送并取得 Build/CodeQL 双绿。CR-013 把真实 `POST/GET /import-tasks/:id/ai-suggestions` 接入财务映射页，并已随 SHA `7d363f6` 取得 Build/CodeQL 双绿。CR-014 在此基础上由服务端核验和持久化四类人工决定及完整 provenance，并已随 SHA `5580ce3` 取得 Build/CodeQL 双绿。CR-015 在第二财务确认页分页展示上述证据并在读取失败时暂停批准；CR-016 又把批准完成后的页面定位到本批正式记录，并提供 `excel-approval/1.0` 不可变快照与记录列表双向跳转。当前候选通过 50 suites / 464 个后端单测、完整 PostgreSQL/Redis 回归 11 suites / 111 个实际执行测试（仓库定义的 3 suites / 14 tests 按既有条件跳过）、完整 Playwright 21/21、44 条 migration 双路径和前后端构建；demo 配置 6/6 与一键故事 1/1 也通过。M0-M8 的非生产工程与合成验收保持有效：AI 只生成受控建议，财务批准前没有正式可见记录，报告数字只来自 canonical ReportSnapshot、Decimal 和固定查询。CR-015 本地提交 `2a59509` 与 CR-016 本地提交 `435acce` 因三次 push 失败及随后 DNS 探测失败尚待推送和同 SHA 远端验证；三次人工演练仍为 `NOT_RUN`，目标 Linux Staging 为 `EXTERNAL_RESOURCE_NEEDED`，财务/OCR/AI 真值为 `REAL_SAMPLE_NEEDED`，项目负责人 UAT 尚未达到 `OWNER_UAT_VERIFIED`，因此本项目**不是 production-ready**。
...
| R11 最终交接 | `in_progress` | CR-010 至 CR-014 已推送且远端全绿；CR-015/CR-016 本地 21/21 E2E 通过，GitHub 连接连续三次失败，尚待正常推送和同 SHA 验证；三次人工演练未执行，不 merge、不转 Ready |
...
- CR-012 Build and acceptance：[run 29828098638](https://github.com/sizhedeng905-sys/FINANCE-AGENT/actions/runs/29828098638)，两个 job 全部成功
- CR-012 CodeQL：[run 29828098718](https://github.com/sizhedeng905-sys/FINANCE-AGENT/actions/runs/29828098718)，成功
- CR-013 Build and acceptance：[run 29831004356](https://github.com/sizhedeng905-sys/FINANCE-AGENT/actions/runs/29831004356)，两个 job 全部成功
...
- CR-013 把后端已有的 Excel AI 建议真实接入财务映射页，同时将旧 `/generate-suggestions` 明确恢复为“新字段定义候选”语义。财务可查看模板、理由、warning、证据、Mock/Provider/模型/Prompt、AI Task 和内容哈希，逐列采纳/拒绝/人工修改/忽略；采纳只改本页 Select 草稿，不自动保存、复用 Profile、校验、跳转或入账。跨模板响应、未知字段、503 和会话切换均失败关闭且不清除人工选择。专项 2/2、完整 Playwright 20/20 与后端 unit 464/464 通过；详见 [`docs/汇报/CR_013_EXCEL_AI_ADVISORY_DRAFT_BRIDGE_2026-07-21.md`](docs/汇报/CR_013_EXCEL_AI_ADVISORY_DRAFT_BRIDGE_2026-07-21.md)。
- CR-014 将页面决定升级为服务端可核验事实：保存映射必须携带 expected task/review version；携带 AI provenance 时，服务端在同一事务内重算输出哈希、校验最新 AI task、冻结模板版本、来源证据、转换白名单和采纳/修改语义，并记录四类决定、操作者、最终字段及 review revision。旧输出、哈希篡改、跨任务/模板、目标字段篡改、重复使用和并发保存均被拒绝，查询接口按页返回审计摘要，audit/ledger 同事务写入且 BusinessRecord 为 0。详见 [`docs/汇报/CR_014_EXCEL_AI_REVIEW_PROVENANCE_2026-07-21.md`](docs/汇报/CR_014_EXCEL_AI_REVIEW_PROVENANCE_2026-07-21.md)。
- CR-015 在第二财务确认页读取上述分页审计摘要，区分 AI 建议、人工决定与最终映射，并展开冻结模板、转换、evidence、AI Task、输出和版本向量哈希。任务/账号/分页变化的晚到响应不会覆盖当前页；证据读取失败时最终批准禁用，批准前正式记录仍为 0。Excel AI 专项 3/3、完整 Playwright 21/21、390px 页面宽度和 teardown 零文件残留均通过；详见 [`docs/汇报/CR_015_EXCEL_AI_REVIEW_CONFIRMATION_UI_2026-07-21.md`](docs/汇报/CR_015_EXCEL_AI_REVIEW_CONFIRMATION_UI_2026-07-21.md)。
- CR-016 修复批准完成后落入全局记录列表的可核对性缺口：URL、Store 和真实 API 现在都保留 `importTaskId`，记录页只显示本批正式记录，并可回到确认页查看批准人、记录数、review revision 与四类哈希。Store 同时恢复既有 `dataLayer` 参数传递。红灯复现后，周五故事 1/1、Excel 4/4、完整 Playwright 21/21、runtime 4/4 与 build 通过；详见 [`docs/汇报/CR_016_EXCEL_APPROVAL_EVIDENCE_AND_RECORD_SCOPE_2026-07-21.md`](docs/汇报/CR_016_EXCEL_APPROVAL_EVIDENCE_AND_RECORD_SCOPE_2026-07-21.md)。
...
| 后端 Jest | `passed` | 本地全量 50/50 suites，464/464 tests |
| PostgreSQL 集成 | `passed` | 强制 Redis 全量 14/14 suites，124/124 tests；含 ReportSnapshot/Claim、AI 租约、step-up、retention、Excel/OCR 严格批准、Redis 共享控制、暂存攻击和大表 Worker 恢复 |
| 浏览器 E2E | `passed` | Playwright 18/18；含真实 API 服务端翻页与周五 Excel 到经营快照故事线 |
| 周五演示命令 | `passed_locally` | demo 配置/攻击 6/6，reset/verify、API/Web smoke、一键故事 1/1；三次人工演练仍为 `NOT_RUN` |
| 前端运行时配置 | `passed` | 4/4；缺失/非法模式、危险 URL 和路径逃逸均失败关闭 |
| Prisma | `passed` | generate/validate/build 与隔离 `_test` 库 44/44 migrations |
...
| Prompt 执行、provenance 与系统登记 | `ENGINEERING_VERIFIED` | 版本化用户模板真实进入请求；受控 JSON、输出 Schema、执行向量和脱敏调用台账覆盖老板助手、Excel/OCR 与报告；系统登记可在空库幂等初始化并由 API/Worker 启动校验，真实模型准确率和生产授权仍开放 |
...
| 最新已推送 GitHub Build | `passed` | CR-014 SHA `5580ce3` 的 run `29834746500` 两个 job 全部成功；CR-015/CR-016 因 GitHub 网络阻断尚无新 SHA 证据 |
| 最新 GitHub CodeQL | `passed` | CR-014 SHA `5580ce3` 的 run `29834746264` 成功；CR-015/CR-016 因 GitHub 网络阻断尚无新 SHA 证据 |
...
|   |-- prisma/             # Schema、44 条 migration 和 seed
