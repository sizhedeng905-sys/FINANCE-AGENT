...
| AI 映射补充 M0-M8 | `M0-M5_passed / M6-M8_in_progress` | OCR 与 Excel 均完成 evidence/revision/revalidation、双人批准快照和事务幂等入账；Excel 部分发布 P0 已关闭 |
...
| PostgreSQL 集成 | `passed` | 全量 9/9 suites，96/96 tests；含 AI 租约/重试/状态变化、step-up 并发防重放、retention lease/legal hold、Excel/OCR 严格批准和大表 Worker 恢复 |
| 浏览器 E2E | `passed` | Playwright 17/17；含真实 API 服务端翻页 20→5 行 |
| 前端运行时配置 | `passed` | 4/4；缺失/非法模式、危险 URL 和路径逃逸均失败关闭 |
| Prisma | `passed` | generate/validate/build 与隔离 `_test` 库 37/37 migrations |
| Migration 路径 | `passed` | M5.2 已验证空库 37 条及 36→37 升级；205 indexes、82 foreign keys |
...
| Repository hygiene | `passed` | 693 个 tracked/candidate 文件通过；真实数据、模型、secret、构建产物和本机供应链证据排除；提交前全量与 staged 门禁均执行 |
