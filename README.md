...
当前仓库已经从前端原型推进到 React 前端、NestJS 后端、PostgreSQL 数据库、异步 Excel/OCR、结构化 AI Claim、本地模型控制面和 Staging 工程。R1-R8.9 已完成前端真实性、日志、容量、恢复、镜像身份、财务并发/精度/幂等、数据生命周期、step-up 和分层 CI 的工程修复；R9.1-R9.3B 又把登录、上传和模型执行控制迁到 Redis 共享原子边界，并缩短大批量确认的最终发布事务。M0-M8 已完成非生产工程与合成验收：Excel/OCR 只生成受控建议，人工修订会使旧校验失效，正式批准要求另一名财务、当前校验哈希和不可变批准快照；Excel 按 H01 每个有效明细行生成一条记录，任何阻断错误都会使整批不发布；报告数字只来自 canonical ReportSnapshot、Decimal 和固定查询。当前本地候选通过 428 个后端单测、114 个 PostgreSQL/Redis 集成测试和 17 个 Playwright；R10 L0 的常驻 Qwen/Paddle 合成推理与鉴权也已验证。受保护 Prompt Catalog 仍为空；目标 Linux Staging、正式职责分离、财务/OCR/AI 真值和人工签字也未完成，因此本项目**不是 production-ready**。
...
- R9.1 已把登录口令限流从单进程状态升级为 Redis Lua 原子共享控制：用户名/IP 只以 SHA-256 键出现，租约使用 Redis 时钟，实例崩溃可自动回收，断连失败关闭且不签发 JWT。双实例 100 路并发、重启保持、租约恢复和断连攻击 4/4 通过；后端 419/419 unit、PostgreSQL/Redis 101/101 integration 和 build 通过。详见 [`docs/R9_1_SHARED_LOGIN_RATE_LIMIT_REPORT_2026-07-21.md`](docs/R9_1_SHARED_LOGIN_RATE_LIMIT_REPORT_2026-07-21.md)。
...
- R9.3A 在全量回归中复现 PostgreSQL WAL checkpoint 令 30,196 行最终发布事务触发 Prisma `P2028`；旧逻辑错误进入终态 `confirmation_failed`。现在 `P2028/P2034` 与短断、连接池超时一样释放租约并由 Worker 有界恢复，暂存记录保持不可见，最终仍只发布一次。P1001/P2028/P2034 定向 3/3、最终 13 suites / 113 integration 通过；30,196 行在真实超时恢复后 167.004 秒完成，距 180 秒预算余量较小，目标磁盘与 PostgreSQL 参数仍须 H13 容量验收。详见 [`docs/R9_3A_IMPORT_CONFIRMATION_TRANSIENT_RECOVERY_REPORT_2026-07-21.md`](docs/R9_3A_IMPORT_CONFIRMATION_TRANSIENT_RECOVERY_REPORT_2026-07-21.md)。
- R9.3B 针对远端 run `29768468874` 中“30,196 行已全部暂存但 180 秒内未完成发布”的红灯，把重复全表计数改为租约围栏增量，并将全量哈希重算移到带心跳的发布预检；最终事务仍重验账号、权限、模板、来源、版本、数量和快照哈希。强制 Redis 全量 13/13 suites、114/114 tests 通过，30,196/49,999 行分别为 25.502/42.954 秒；修复提交 `cc033d4` 与 R11 文档提交 `9e889bb` 均已推送，最新 head/checks 以 Draft PR #4 为准。详见 [`docs/R9_3B_IMPORT_PUBLICATION_TRANSACTION_HARDENING_REPORT_2026-07-21.md`](docs/R9_3B_IMPORT_PUBLICATION_TRANSACTION_HARDENING_REPORT_2026-07-21.md)。
...
| PostgreSQL 集成 | `passed` | 强制 Redis 全量 13/13 suites，114/114 tests；含 ReportSnapshot/Claim、AI 租约、step-up、retention、Excel/OCR 严格批准、Redis 共享控制和大表 Worker 恢复 |
...
| 大批量 Excel | `engineering_passed / target_capacity_pending` | R9.3B 强制 Redis 全量中 30,196/49,999 行为 25.502/42.954 s；最终记录、动态值、金额、audit、ledger 和日报闭环，目标磁盘/WAL/p95/p99 仍待 H13 |
...
| R8.2-R8.7 条件验收自动化 | `engineering_verified / blocked_external` | scheduled/manual Staging、恢复、回滚、Python adapter 与 GPU L0 workflow 已建立；本机 release/日志/同 manifest rollback、Python 8/8 和真实模型 L0 通过；目标 self-hosted 环境仍受 H13/H14 阻断 |
...
| R9.3 共享模型执行门 | `engineering_verified_locally` | AI/OCR/推理健康探针共用 Redis FIFO 并发预算；6/6 双实例/租约/超时/故障测试，当前候选 428/428 unit、114/114 integration；目标多实例 Staging 仍待 H13 |
| R9.3A 导入事务恢复 | `engineering_verified_locally` | P2028/P2034 释放租约并有界恢复，暂存数据不提前可见；当前候选 114/114 integration；目标存储性能余量待 H13 |
| R9.3B 发布事务收敛 | `engineering_verified_locally / remote_ci_required` | 哈希预检带租约心跳，最终事务保留完整围栏；P2028 最终发布恢复定向通过，30,196/49,999 行 25.502/42.954 s；最新 head/checks 见 Draft PR #4 |
