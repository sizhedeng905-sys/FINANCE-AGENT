...
本地开发可用单进程 API 和本地文件存储。生产配置中的全局请求限流、登录限流和上传准入已使用 Redis 共享原子控制；模型并发/排队仍有进程内状态，因此提供的 Staging 拓扑继续固定为单 API、单 Worker。完成 R9.3 多实例故障测试前不得横向扩容。
