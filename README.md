...
当前仓库已经从前端原型推进到 React 前端、NestJS 后端、PostgreSQL 数据库、异步 Excel/OCR、结构化 AI Claim、本地模型控制面和 Staging 工程。CR-002 至 CR-010 已关闭 Excel staging、Prompt provenance、治理、production-safe AI 系统登记和后端运行镜像供应链问题。CR-011 建立周五演示级“Excel 到经营报告”合成 E2E；CR-012 又把它整理为只允许本机 `finance_agent_test`、可 reset/verify/start/test 的[周五演示交付包](docs/deliveries/2026-07-24/README.md)，两者已随 SHA `66749b3` 推送并取得 Build/CodeQL 双绿。CR-013 进一步把真实 `POST/GET /import-tasks/:id/ai-suggestions` 接入财务映射页，显示模板、证据、warning、Provider/模型/Prompt 与内容哈希，并支持逐列采纳、拒绝、人工修改和忽略；所有动作只进入当前页面草稿，跨模板和 Provider 故障失败关闭，保存前正式记录保持 0。当前候选通过 50 suites / 464 个后端单测、14 suites / 124 个 PostgreSQL/Redis 集成测试、20 个 Playwright、43 条 migration 双路径和前后端构建；demo 配置 6/6 与一键故事 1/1 也通过。M0-M8 的非生产工程与合成验收保持有效：AI 只生成受控建议，财务批准前没有正式可见记录，报告数字只来自 canonical ReportSnapshot、Decimal 和固定查询。CR-013 远端 CI 尚待提交后验证，人工审核决定的服务端 provenance 持久化是下一工程块；三次人工演练仍为 `NOT_RUN`，目标 Linux Staging 为 `EXTERNAL_RESOURCE_NEEDED`，财务/OCR/AI 真值为 `REAL_SAMPLE_NEEDED`，项目负责人 UAT 尚未达到 `OWNER_UAT_VERIFIED`，因此本项目**不是 production-ready**。
...
| R11 最终交接 | `in_progress` | CR-010 至 CR-012 已推送且远端全绿；CR-013 Excel AI 人工草稿桥接本地 20/20 E2E 通过，下一项是审核决定/provenance 持久化；三次人工演练未执行，不 merge、不转 Ready |
...
| 最新已推送 GitHub Build | `passed` | CR-012 SHA `66749b3` 的 run `29828098638` 两个 job 全部成功；CR-013 尚待提交后取得同 SHA 证据 |
| 最新 GitHub CodeQL | `passed` | CR-012 SHA `66749b3` 的 run `29828098718` 成功；CR-013 尚待提交后取得同 SHA 证据 |
