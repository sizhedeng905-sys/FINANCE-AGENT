...
- R9.3B 针对远端 run `29768468874` 中“30,196 行已全部暂存但 180 秒内未完成发布”的红灯，把重复全表计数改为租约围栏增量，并将全量哈希重算移到带心跳的发布预检；最终事务仍重验账号、权限、模板、来源、版本、数量和快照哈希。强制 Redis 全量 13/13 suites、114/114 tests 通过，30,196/49,999 行分别为 25.502/42.954 秒；本地提交 `cc033d4` 因两次 GitHub 网络失败尚待推送与远端复跑。详见 [`docs/R9_3B_IMPORT_PUBLICATION_TRANSACTION_HARDENING_REPORT_2026-07-21.md`](docs/R9_3B_IMPORT_PUBLICATION_TRANSACTION_HARDENING_REPORT_2026-07-21.md)。
...
| R9.3B 发布事务收敛 | `engineering_verified_locally / remote_ci_pending` | 哈希预检带租约心跳，最终事务保留完整围栏；P2028 最终发布恢复定向通过，30,196/49,999 行 25.502/42.954 s；提交 `cc033d4` 待网络恢复后推送 |
