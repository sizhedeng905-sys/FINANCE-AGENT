...
| AI 映射补充 M0-M8 | `M0-M3_passed / M4_in_progress` | Excel 列摘要与 OCR 有界证据均只生成建议；OCR 已校验项目模板、字段、source/evidence 绑定与跨页冲突，人工修订、重新校验和证据 UI 尚在实施 |
...
- H01 已决定每个有效明细行一条记录、汇总由程序计算且汇总/明细不得双计；汇总行真实样例和正式签字仍缺。H02 决定白名单负数、保留历史的财务更正和第一版不关账，但白名单与报表重述仍缺；H07 决定每个费用明细为主记录、多附件、缺必填附件阻断和人工覆盖留痕，但必填模板清单仍缺。实现前继续采用 R6.6 保守行为，详见 [`docs/R6_6_H01_H02_H07_BEHAVIOR_MATRIX_2026-07-18.md`](docs/R6_6_H01_H02_H07_BEHAVIOR_MATRIX_2026-07-18.md)。
...
| Migration 路径 | `passed` | M3.2 已验证空库 34 条及 33→34 升级 |
...
| OCR AI 分类/映射 | `engineering_partial` | M4.1 已验证受限证据摘要、项目模板白名单、source/evidence 绑定、跨页冲突转人工和零正式记录；review revision、重新校验与 bbox UI 待完成 |
...
| Repository hygiene | `passed` | 680 个 tracked/candidate 文件通过；真实数据、模型、secret、构建产物和本机供应链证据排除；提交前全量与 staged 门禁均执行 |
